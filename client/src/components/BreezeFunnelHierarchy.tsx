...
export function buildPriorPeriodActivitySequence(activityCount: number) {
  return Array.from({ length: Math.max(0, Math.min(50, Math.trunc(activityCount))) }, (_, index) => `Recorded activity ${index + 1}`);
}
