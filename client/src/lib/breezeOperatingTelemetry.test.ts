...
describe("Breeze operating telemetry", () => {
  it("keeps channel engagement and email outreach as distinct operating stages", () => {
    ...
    expect(BREEZE_OPERATING_TELEMETRY.find(item => item.stage === "Email outreach")?.status).toBe("Email");
  ...
  it("represents SiteID and affiliate as operating statuses rather than conversion metrics", () => {
    expect(BREEZE_STAGE_STATUS[0]?.status).toBe("Not installed");
    expect(BREEZE_STAGE_STATUS[1]?.detail).toBe("No sign-up results are shown");
  });
});
